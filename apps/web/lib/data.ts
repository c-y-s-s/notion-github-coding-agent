...
export async function getBenchmarkCaseResults(caseId: string) {
  ...
  const { data, error } = await adminDb()
    ...
    .select(
      "*,benchmark_runs(model,prompt_version,dataset_version,created_at,status)",
    )
    .eq("case_id", caseId)
    .order("created_at", { ascending: false })
    .limit(20);
...
export async function getBenchmarkRunWithResults(id: string) {
  ...
  const db = adminDb();
  const [{ data: run, error }, { data: results, error: resultsError }] =
    await Promise.all([
  ...
  if (error || resultsError)
    throw serviceError("讀取 Benchmark 比較資料失敗", error ?? resultsError);
...
export async function getDemoStory() {
  ...
  const [
    ...
    db
      ...
      .select("*")
      .or(
        `id.eq.${replay.parent_run_id},parent_run_id.eq.${replay.parent_run_id}`,
      )
      .order("created_at"),
    db
      .from("artifacts")
      .select("*")
      .in("agent_run_id", [replay.parent_run_id, replay.id])
      .order("created_at"),
    db
...
export async function getRun(id: string) {
  ...
  if (!hasSupabaseEnv())
    return {
      run: demoRuns.find((x) => x.id === id) ?? demoRuns[0],
      steps: [],
      artifacts: [
        {
          id: "demo-artifact",
          type: "diff",
          content:
            "示範模式：設定 Supabase 並啟動 Worker 後，即可產生真實的程式碼修改。",
          metadata: {},
        },
      ],
      evaluation: null,
  ...
  const [
    ...
    db.from("agent_runs").select("*").eq("id", id).single(),
    db
      .from("agent_run_steps")
      .select("*")
      .eq("agent_run_id", id)
      .order("sequence"),
    db.from("artifacts").select("*").eq("agent_run_id", id).order("created_at"),
    db
      .from("agent_evaluations")
      .select("*")
      .eq("agent_run_id", id)
  ...
  if (error) throw serviceError("讀取 Agent 執行詳情失敗", error);
  const relatedIds = [run.parent_run_id, run.id].filter(Boolean);
  const { data: related } = await db
    .from("agent_runs")
    .select(
      "id,parent_run_id,replay_mode,status,model,prompt_version,base_commit_sha,attempt_number,token_usage,started_at,finished_at,error_code",
    )
    .or(
      `id.in.(${relatedIds.join(",")}),parent_run_id.eq.${run.parent_run_id ?? run.id}`,
    )
    .order("created_at");
  return {
    run,
...
export async function getTaskDetail(id: string) {
  ...
  if (!hasSupabaseEnv()) {
    return {
      task: demoTasks.find((item) => item.id === id) ?? null,
      pullRequests: [],
      latestRun: null,
      artifacts: [],
    };
  ...
  if (!task)
    return { task: null, pullRequests: [], latestRun: null, artifacts: [] };
