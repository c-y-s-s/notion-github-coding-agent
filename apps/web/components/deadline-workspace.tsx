...
import { orderedSprints, sprintLabel } from "@/lib/sprint-display";
...
export function DeadlineWorkspace({ tasks, sprints, today }: { tasks: WorkItem[]; sprints: Sprint[]; today: string }) {
  ...
  const initialWeekStart = startOfWeek(parseDate(today));
  ...
  return <section className="section card deadline-workspace">
    ...
    <div className="date-toolbar">
      <select className="date-input" aria-label="Sprint 篩選" value={sprintId} onChange={event => setSprintId(event.target.value)}><option value="all">所有 Sprint</option><option value="backlog">Backlog</option>{orderedSprints(sprints).map(sprint => <option key={sprint.id} value={sprint.id}>{sprintLabel(sprint, sprints)}</option>)}</select>
...
function formatDate(value: string) { const date = parseDate(value); return `${date.getMonth() + 1}月${date.getDate()}日 週${"日一二三四五六"[date.getDay()]}`; }
