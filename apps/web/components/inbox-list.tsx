...
export function InboxList({ items, configured }: { items: WorkItem[]; configured: boolean }) {
  ...
  return <section className="section card">
    ...
    {visible.length === 0 ? <EmptyState title={emptyTitle(view)} description={emptyLabel(view)} /> : visible.map(item => <article className="inbox-item" key={item.id}>
...
function emptyTitle(view: InboxView) { return ({ pending: "收件匣已清空", needs_info: "沒有待補資訊", ignored: "沒有忽略紀錄" } as const)[view]; }
