...
export function BenchmarkRunForm({ defaultModel, caseId }: { defaultModel: string; caseId?: string }) {
  ...
  return <form className="benchmark-run-form" onSubmit={submit}><label><span>模型</span><input value={model} onChange={event => setModel(event.target.value)} required pattern="[a-zA-Z0-9._-]+" /></label><label><span>Prompt</span><select value={promptVersion} onChange={event => setPromptVersion(event.target.value)}><option value="v1">v1 基準</option><option value="v2">v2 證據導向</option></select></label><button className="button" disabled={busy}>{busy ? "排程中…" : caseId ? "重新執行此案例" : "執行完整 Benchmark"}</button>{message && <small className="muted">{message}</small>}</form>;
}
