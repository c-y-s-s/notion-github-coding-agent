...
export async function POST(request: Request) {
  ...
  console.info("[github-webhook] received", { delivery, event });
  if (!verifyGitHubSignature(raw, request.headers.get("x-hub-signature-256"))) {
    console.warn("[github-webhook] invalid signature", { delivery, event });
    return failure("Invalid GitHub signature", 401);
  }
  ...
  try {
    ...
    console.info("[github-webhook] completed", { delivery, event });
    return ok({ accepted: true }, 202);
  } catch (error) { console.error("[github-webhook] failed", { delivery, event, error: String(error) }); await db.from("sync_events").update({ status: "failed", last_error: String(error) }).eq("provider", "github").eq("provider_event_id", delivery); return failure("Webhook processing failed", 500); }
}
