...
export async function POST(request: Request) {
  ...
  try {
    if (event === "issues" && payload.issue && payload.repository) {
      ...
      const { data: repo } = await db.from("repositories").select("id,project_id").eq("github_node_id", repository.node_id).maybeSingle();
    ...
    if (event === "pull_request" && payload.pull_request && payload.repository) {
      const pr = payload.pull_request; const { data: repo } = await db.from("repositories").select("id").eq("github_node_id", payload.repository.node_id).maybeSingle();
