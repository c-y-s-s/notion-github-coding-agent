...
export default async function Tasks() {
  ...
  return <>
    <PageHeader eyebrow="正式工作項目" title="任務" description="依 Sprint 安排工作；狀態與 Deadline 會同步回 Notion。" />

    {tasks.length === 0 ? <section className="section card"><EmptyState title="目前沒有正式任務" description="先從 GitHub 收件匣審核 Issue，或在 Notion 建立任務。" actionHref="/inbox" actionLabel="前往 GitHub 收件匣" /></section> : <TaskBoard tasks={tasks} sprints={sprints} today={taipeiDate()} />}
  </>;
}
